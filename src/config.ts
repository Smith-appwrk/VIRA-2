...
const KB_REVIEW_USERS = (process.env.KB_REVIEW_USERS || '').split('|').map(name =>
  name.toLowerCase().replaceAll(' ', '')
).filter(name => name.length > 0);
...
export const CONFIG = {
  ...
  KNOWLEDGE_REVIEW_GROUP_ID: process.env.KNOWLEDGE_REVIEW_GROUP_ID || '19:94e839e49810435bb48afaa069113a93@thread.v2',
  MONITORED_GROUP_IDS: process.env.MONITORED_GROUP_IDS,
  DAILY_ANALYSIS_TIME: process.env.DAILY_ANALYSIS_TIME || "19:07"
};
